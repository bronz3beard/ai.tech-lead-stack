...
function NewSessionModal({ projectId, onCreated, onClose }: NewSessionModalProps) {
...
function SessionCard({ session }: { session: ReviewSession }) {
  ...
  const passCount = session.gateResults.filter((g) => g.status === 'pass').length;

  return (
    <button
      onClick={() => router.push(`/design-review/${session.id}`)}
      className="w-full text-left p-4 bg-zinc-900 hover:bg-zinc-800/80 border border-zinc-800 hover:border-zinc-700 rounded-xl transition-all group space-y-3"
    >
