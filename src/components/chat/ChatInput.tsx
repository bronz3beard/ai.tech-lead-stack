...
export default function ChatInput({
  ...
  const workflows = useMemo(() => {
    if (session?.user && (session.user as any).role) {
      return getWorkflowsForRole((session.user as any).role);
    }
    return [];
  }, [session]);
    ...
    <div className="relative max-w-4xl mx-auto">
          ...
          <ul>
            {workflows
              ...
              .map((w) => (
                ...
                >
                  <span className="font-mono text-indigo-400 mr-4 shrink-0 w-[200px]">
                    /{w.name}
                  </span>
      ...
      <div className="text-center mt-2 flex items-center justify-center space-x-2">
        ...
        <span className="text-[10px] text-zinc-500 italic">
          AI can make mistakes. Focus is code analysis only.
        </span>
      </div>
    </div>
  );
