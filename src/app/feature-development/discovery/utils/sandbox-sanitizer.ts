...
import { MIDDLEWARE_NOOP_STUB, ASYNC_STORAGE_PATCH_STUB } from '../constants/stubs';
 ...
 */
export async function sanitizeSandboxEnvironment(instance: WebContainer, appDir: string) {
  ...
  try {
    ...
    console.log('[Sanitizer] Injected async-storage-patch.js stability module.');
  } catch (err) {
  ...
  const sanitizeRecursively = async (dir: string) => {
    ...
    for (const file of files) {
      ...
      } else if (file.name.endsWith('.ts') || file.name.endsWith('.tsx') || file.name.endsWith('.js') || file.name.endsWith('.jsx') || file.name.endsWith('.html')) {
        try {
          ...
          if (content.includes('navigator.serviceWorker')) {
            console.log(`[SW Sanitizer] Mocking navigator.serviceWorker inside ${path}`);
  ...
  try {
    ...
    if (isNext) {
      // Force delete any Babel configs to ensure Next.js falls back to its robust WebAssembly (WASM) SWC compiler.
      const babelFiles = ['.babelrc', 'babel.config.js', 'babel.config.json', 'babel.config.mjs', 'babel.config.cjs'];
      for (const bf of babelFiles) {
        try { await instance.fs.rm(bf); } catch {}
        try { await instance.fs.rm(`${appDir}/${bf}`); } catch {}
      }
      ...
      const targetMjs = appDir === '.' ? 'next.config.mjs' : `${appDir}/next.config.mjs`;
      await instance.fs.writeFile(targetMjs, `export default ${safeConfig};`);
      console.log(`[Sanitizer] Exclusively wrote safe next.config.mjs at: ${targetMjs}`);
      ...
      for (const cf of configsToDelete) {
        ...
        try {
          ...
          console.log(`[Sanitizer] Cleaned up competing config file to bypass TS compilation: ${targetToDelete}`);
        } catch {}
      }
    ...
    console.warn('[Sanitizer] Failed to perform Next.js specific sanitization:', err);
  }
