...
if [[ ! -f "$CHECK_FILE" ]]; then
    echo "ℹ️  Mission alignment not recorded yet — skipping this tool run."
    echo "----------------------------------------------------------------------"
    echo "This is the expected state in a project where no agent has run a"
    echo "Tech-Lead Stack skill yet. It is not an installation failure and"
    echo "nothing is broken."
    echo ""
    echo "An agent records alignment by calling the MCP tool"
    echo "'verify_mission_alignment', which writes $CHECK_FILE."
    echo "Once that exists, 'rtk run' proceeds normally."
    echo "----------------------------------------------------------------------"
    exit 1
fi
