...
2. **Pre-Flight Model Contract**: Print model assignments and isolation levels (L0-L3). Check `CLAUDE_CODE_SUBAGENT_MODEL` (warn and cap at L2 if overridden). Resolve the critic with `./.ai/rtk-run run resolve-critic --writer <vendor> --writer-model <generator-model>` (Gemini `gemini-cli`/`agy` -> `codex` -> Claude sub-agent/`claude-cli` -> another model -> same model + STRONG `criticAdvisory`); record `criticResolution` in `.loop-out/<runId>/state.json`.
